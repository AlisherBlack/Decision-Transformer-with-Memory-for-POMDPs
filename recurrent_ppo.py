...
class RecurrentPPOMemory:
    ...
    def get_batch_indices(self, batch_size):
        ...
        indices = np.random.choice(
            trajectory_starts, batch_size, replace=len(trajectory_starts) < batch_size
        )
        return indices
    ...
    def forward(self, state, hidden=None, sequence_mode=False):
        ...
        if self.rnn_type == "gru":
            if len(features.shape) == 2:
                ...
                rnn_out, hidden_out = self.rnn(
                    features.unsqueeze(0), hidden.unsqueeze(1)
                )
            ...
            if len(features.shape) == 2:
                if hidden[0].dim() == 3:
                    ...
                    rnn_out, hidden_out = self.rnn(
                        features.unsqueeze(0), (h0.unsqueeze(1), c0.unsqueeze(1))
                    )
                    rnn_out = rnn_out.squeeze(0)
                else:
                    rnn_out, hidden_out = self.rnn(
                        features.unsqueeze(0),
                        (hidden[0].unsqueeze(1), hidden[1].unsqueeze(1)),
                    )
                    rnn_out = rnn_out.squeeze(0)
            else:
                rnn_out, hidden_out = self.rnn(features, hidden)
...
class RecurrentPPO:
    ...
    def learn(self, total_timesteps, max_ep_len=1000):
        ...
        while timestep < total_timesteps:
            ...
            hidden_state = (
                tuple(h.to(self.device) for h in hidden_state)
                if isinstance(hidden_state, tuple)
                else hidden_state.to(self.device)
            )
            ...
            while not done and episode_timesteps < max_ep_len:
                action, value, log_prob, next_hidden = self.choose_action(
                    obs, hidden_state
                )
                ...
                if (
                    ...
                    or episode_timesteps >= max_ep_len
                    or timestep % self.sequence_length == 0
                ):
                    if not done:
                        with torch.no_grad():
            ...
            avg_reward = (
                np.mean(self.reward_history[-50:])
                if len(self.reward_history) >= 50
                else np.mean(self.reward_history)
            )

            print(
                f"Episode {episode}, reward: {episode_reward:.2f}, avg_reward (last 50): {avg_reward:.2f}, timesteps: {timestep}/{total_timesteps}"
            )
            ...
            if episode_reward > best_reward:
                ...
                if (
                    best_reward >= 500.0
                    and len(self.reward_history) >= 5
                    and np.mean(self.reward_history[-5:]) >= 490.0
                ):
                    print(
                        f"🎉 Environment solved! Stopping training early at timestep {timestep}"
                    )
                    break
    ...
    def update(self):
        ...
        for _ in range(self.epochs):
            for i in range(0, len(states), self.sequence_length):
                ...
                hidden = (
                    tuple(h.to(self.device) for h in hidden)
                    if isinstance(hidden, tuple)
                    else hidden.to(self.device)
                )

                action_probs, values, _ = self.policy(
                    seq_states, hidden, sequence_mode=True
                )
                ...
                if self.update_count % 100 == 0:
                    with torch.no_grad():
                        approx_kl = (
                            ((seq_old_logprobs - new_logprobs) ** 2).mean().item()
                        )
                    ...
                    print(
                        f"Update {self.update_count} - Loss: {loss.item():.4f}, "
    ...
    def collect_trajectories(self, num_trajectories, render=False):
        ...
        for traj_idx in range(num_trajectories):
            ...
            while not done:
                ...
                if isinstance(raw_obs, dict):
                    raw_obs = (
                        ...
                        else np.array(list(raw_obs.values()))
                    )
            ...
            print(
                f"Trajectory {traj_idx + 1}/{num_trajectories}, reward: {episode_reward}"
            )
    ...
    def save_trajectories(self, output_dir, num_trajectories=1000):
        ...
        while saved_count < num_trajectories and attempt_count < max_attempts:
            ...
            if meets_threshold:
                ...
                saved_count += 1
                print(
                    f"Saved trajectory {saved_count}/{num_trajectories} with reward {total_reward:.2f}"
                )
        ...
        if saved_count < num_trajectories:
            print(
                f"Warning: Only collected {saved_count}/{num_trajectories} trajectories "
