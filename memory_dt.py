...
class MemoryDecisionTransformer(nn.Module):
    def __init__(
        ...
        if memory_type == "gru":
            ...
            self.memory = nn.GRU(
                input_size=n_embed, hidden_size=memory_dim, batch_first=True
            )
            ...
            self.memory_proj = nn.Linear(memory_dim, n_embed)
        elif memory_type == "delta_gated":
            self.memory = nn.Sequential(
                nn.Linear(2 * n_embed, memory_dim),
                nn.ReLU(),
                nn.Linear(memory_dim, n_embed),
            )
            self.memory_gate = nn.Sequential(
                nn.Linear(2 * n_embed, n_embed), nn.Sigmoid()
            )
        ...
        if self.memory is not None:
            if self.memory_type == "gru":
                if self.hidden_state is None:
                    ...
                    self.hidden_state.size(1) != batch_size
                    or self.hidden_state.device != state_embeddings.device
                ):
                    self.hidden_state = torch.zeros(
                        1,
                        batch_size,
                ...
                memory_out, self.hidden_state = self.memory(
                    state_embeddings, self.hidden_state
                )
                ...
                if self.hidden_state is None:
                    ...
                    self.hidden_state[0].size(1) != batch_size
                    or self.hidden_state[0].device != state_embeddings.device
                ):
                    h0 = torch.zeros(
                        1,
                        batch_size,
            ...
            elif self.memory_type == "delta_gated":
                prev_embeddings = torch.cat(
                    [state_embeddings[:, :1], state_embeddings[:, :-1]], dim=1
                )
                delta_embeddings = state_embeddings - prev_embeddings
                memory_input = torch.cat([state_embeddings, delta_embeddings], dim=-1)
                memory_out = self.memory_gate(memory_input) * self.memory(memory_input)
...
def train_memory_dt(
    ...
    print(
        f"Dataset stats: total={len(dataset)}, train={train_size}, val={val_size}, state_dim={dataset.state_dim}, actions={dataset.vocab_size}"
    )
    ...
    for epoch in range(n_epochs):
        ...
        epoch_loss = 0
        progress_bar = tqdm(
            train_dataloader, desc=f"Epoch {epoch + 1}/{n_epochs} [Train]"
        )
        ...
        for batch_idx, (states, actions, rtgs, _) in enumerate(progress_bar):
            ...
            loss = criterion(
                action_preds.reshape(-1, dataset.vocab_size), actions.reshape(-1)
            )
        ...
        for episode in range(num_eval_episodes):
                ...
                if isinstance(obs, dict):
                    if "observation" in obs and "mask" in obs:
                        ...
                        processed_obs = np.concatenate([
                            ...
                            else np.array([o], dtype=np.float32)
                            for o in obs.values()
                        ])
                else:
                    processed_obs = obs
            ...
            if isinstance(val_env, VelocityCartPoleEnv) and episode_return >= 450:
                ...
                isinstance(val_env, LiDARMountainCarEnv)
                and done
            ...
            print(
                f"Episode {episode + 1}: Return={episode_return:.1f}, Steps={timestep}"
            )
        ...
        print(
            f"Validation: Mean Return={mean_return:.2f}, Success Rate={success_rate:.2%}"
        )
    ...
    torch.save(
        best_model_state if best_model_state else model.state_dict(), best_model_path
    )
...
def calculate_dataset_stats(dataset_path):
    ...
    try:
        ...
        plt.axvline(
            ...
            color="g",
            linestyle="--",
            label=f"Median: {stats['median_reward']:.2f}",
        )
        plt.axvline(
            stats["reward_p90"],
            color="orange",
            linestyle="--",
            label=f"90th percentile: {stats['reward_p90']:.2f}",
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Execution of dataset operations and model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stats_only",
        action="store_true",
        help="Only print dataset statistics without training",
    )
    parser.add_argument(
        "--env",
        type=str,
        default="velocity_cartpole",
    ...
    parser.add_argument(
        ...
        type=str,
        default="gru",
        choices=["gru", "lstm", "delta_gated", "none"],
        help="Memory type",
    )
