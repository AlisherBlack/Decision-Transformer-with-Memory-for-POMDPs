...
def main():
    ...
    if args.env == "velocity_cartpole":
        ...
        sequence_length = 128
        reward_threshold = (
            450 if args.reward_threshold is None else args.reward_threshold
        )
    else:
    ...
    # Case 2 & 3: Either checkpoint doesn't exist or --collect_data_from_checkpoint=False
        ...
        train_timesteps = (
            300000 if args.env == "velocity_cartpole" else args.train_timesteps
        )
        train_reason = (
            "from scratch" if not checkpoint_exists else "ignoring existing checkpoint"
        )
        print(
            f"Training recurrent PPO agent {train_reason} on {args.env} for {train_timesteps} timesteps..."
        )
