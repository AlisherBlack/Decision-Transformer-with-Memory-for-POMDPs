...
def parse_args():
    ...
    parser.add_argument(
        "--model_path", type=str, required=True, help="Path to the trained PPO model"
    )
    parser.add_argument(
        "--num_episodes", type=int, default=5, help="Number of episodes to visualize"
    )
    parser.add_argument(
        "--hidden_dim", type=int, default=64, help="Hidden dimension used in the model"
    )
    parser.add_argument(
        "--rnn_type",
        type=str,
    ...
    parser.add_argument(
        "--record", action="store_true", help="Record video of the agent performance"
    )
...
def load_model(model_path, env, hidden_dim, rnn_type):
    ...
    if isinstance(env.action_space, gym.spaces.Discrete):
        ...
        raise ValueError(
            "Only discrete action spaces supported for now. But if you want to use continuous action spaces, you can do it by yourself =)"
        )
    ...
    if actual_hidden_dim != hidden_dim:
        print(
            f"WARNING: Determined hidden_dim={actual_hidden_dim} from model, overriding argument hidden_dim={hidden_dim}"
        )
...
def evaluate_agent(env, policy, num_episodes, delay=0.05):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            print(
                f"Step {timestep + 1}: Action={action.item()}, Value={value.item():.3f}"
            )
