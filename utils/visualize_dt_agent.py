...
def parse_args():
    ...
    parser.add_argument(
        "--num_episodes", type=int, default=5, help="Number of episodes to visualize"
    )
    parser.add_argument(
        "--context_length",
        type=int,
    ...
    parser.add_argument(
        "--memory_dim", type=int, default=64, help="Hidden dimension of memory state"
    )
    parser.add_argument(
        "--n_layer", type=int, default=3, help="Number of transformer layers"
    )
    parser.add_argument(
        "--n_head", type=int, default=4, help="Number of attention heads"
    )
    parser.add_argument("--n_embed", type=int, default=128, help="Embedding dimension")
    parser.add_argument(
        "--target_return",
        type=float,
        default=None,
...
def load_model(model_path, env, args):
    ...
    if "state_encoder.weight" in state_dict:
        ...
        if actual_n_embed != args.n_embed:
            print(
                f"WARNING: Determined n_embed={actual_n_embed} from model, overriding argument n_embed={args.n_embed}"
            )
    ...
    if memory_type is not None and "memory_proj.weight" in state_dict:
        ...
        if actual_memory_dim != args.memory_dim:
            print(
                f"WARNING: Determined memory_dim={actual_memory_dim} from model, overriding argument memory_dim={args.memory_dim}"
            )
    ...
    if layer_count > 0 and layer_count != args.n_layer:
        print(
            f"WARNING: Determined n_layer={layer_count} from model, overriding argument n_layer={args.n_layer}"
        )
        n_layer = layer_count
    else:
    ...
    if "transformer.layers.0.self_attn.in_proj_weight" in state_dict:
        in_proj_weight_shape = state_dict[
        ...
        if actual_n_head != args.n_head:
            print(
                f"WARNING: Determined n_head={actual_n_head} from model, overriding argument n_head={args.n_head}"
            )
    ...
    print(
        f"Model parameters: n_embed={n_embed}, n_layer={n_layer}, n_head={n_head}, memory_dim={memory_dim}"
    )
    print(
        f"action_head structure: {'Sequential' if action_head_sequential else 'Linear'}"
    )
...
def evaluate_agent(
    env, model, num_episodes, context_length=20, target_return=None, delay=0.05
):
